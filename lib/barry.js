...
self.criteria = {
    ...
    percentScale: function (mods) {
        ...
        self.log('======================================== :: index');
        self.log("(j.corns) (user.id*salt % 100)", (self.data.id * salt % 100));
        ...
        self.log("(j.corns) Math.min((user.id*salt)%100, percentMin)", Math.min((self.data.id * salt) % 100, percentMin));
        self.log("(j.corns) Math.max((user.id*salt), percentMax)%100", Math.max((self.data.id * salt) % 100, percentMax));
